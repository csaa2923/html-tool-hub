...
export default async function handler(req, res) {
  ...
  if(!auth) return res.status(401).json({error:'Nicht angemeldet'});
  try {
    if (req.method === 'GET') {
      const all=await readTools();
      return res.status(200).json(auth.role==='admin' ? all : all.filter(x=>x.visibility!=='private'));
    }
    ...
    if (action === 'add') {
      ...
      if(!t) return res.status(404).json({error:'Tool nicht gefunden'});
    ...
    console.error(e);
    return res.status(500).json({error:'Zentrale Speicherung nicht verfügbar. Blob-Verbindung bzw. Read-Write-Token prüfen.'});
  }
}